...
void SteamMultiplayerPeer::network_messages_session_failed_scb(SteamNetworkingMessagesSessionFailed_t *call_data) {
	...
	ERR_PRINT(String("Connection terminated. See ESteamNetConnectionEnd for reason ") + String::num(reason));
	int connection_state = info.m_eState;
	ERR_PRINT(String("See ESteamNetworkingConnectionState for connection state is ") + String::num(connection_state));
	...
	ERR_PRINT(String("Failed to connect to user SteamID ") + String::num(other_end_steamid));
	/// Human-readable, but non-localized explanation for connection termination or problem. (m_szEndDebug)
	/// Debug description.  This includes the connection handle, connection type and peer information, and the app name. (m_szConnectionDescription)
	ERR_PRINT(String::utf8(info.m_szEndDebug) + String("; ") + String::utf8(info.m_szConnectionDescription));
	DEBUG_DATA_SIGNAL_V("network_messages_session_failed_scb", reason);
	emit_signal("network_session_failed", other_end_steamid, reason, connection_state);
}
